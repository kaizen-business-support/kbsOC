...
| Migration Prisma sur DB de prod | Champ nullable avec default null → migration sans risque (pas de backfill). Décision validée : **tous les utilisateurs existants** (même ceux avec `last_login != null`) verront le tour à leur prochaine connexion. Pas d'`UPDATE` post-migration. |
