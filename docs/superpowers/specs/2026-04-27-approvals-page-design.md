...
```ts
{
  companyId: req.companyId,       // multi-tenant
  status: 'PENDING',
  completedAt: null,
  OR: [
...
```ts
{
  ...
  currency: string
  stepName: string
  stepLabel: string              // depuis policyStep.stepLabel ou stepName
  stepType: string               // depuis policyStep.stepType
  allowedActions: string[]       // depuis policyStep.allowedActions
